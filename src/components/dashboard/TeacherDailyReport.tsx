...
export default function TeacherDailyReport() {
  ...
  // Get unique sessions from the records
  const sessions = Array.from(new Set(records.map(r => r.sessionId))).map(id => {
    const rec = records.find(r => r.sessionId === id);
    return { id, time: rec?.markedAt.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
  });
  ...
  const filteredRecords = records.filter(r => {
    ...
    if (filter === 'verified') return matchesSearch && matchesSession && r.verified && r.faceVerified;
    if (filter === 'flagged') return matchesSearch && matchesSession && (!r.verified || !r.faceVerified);
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          {sessions.length > 0 && (
            <select
              ...
              className="bg-white border border-brand-border rounded-xl px-4 py-2.5 text-xs font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-brand-primary/10 transition-all"
            >
              <option value="all">All Sessions</option>
              {sessions.map(s => (
                <option key={s.id} value={s.id}>Session at {s.time}</option>
              ))}
            </select>
          )}
